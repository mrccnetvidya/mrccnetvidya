...
class process {
    ...
    protected function find_standard_fields(): void {
        $this->standardfields = array('id', 'username', 'email', 'emailstop','userstatus',
    ...
    public function process_line(array $line) {
        ...
        if ($existinguser) {
            ...
            ##Edited by vraj@mrccsolution.com
            if (empty($user->userstatus)) {
                $this->upt->track('userstatus', get_string('invaliduserstatus'), 'error');
                $this->upt->track('status', get_string('usernotaddederror', 'error'), 'error');
                $this->userserrors++;
                return;
